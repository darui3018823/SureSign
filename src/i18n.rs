...
fn get_en(key: &str) -> String {
    match key {
        ...
        "enter_country" => "Country Name (2 letter code):".to_string(),
        ...
        "enter_pfx_password" => "PFX Password (leave empty for no password):".to_string(),
        "files_exist_warning" => "Warning: The following files already exist:".to_string(),
        "overwrite_prompt" => "Overwrite? (y/N):".to_string(),
...
fn get_ja(key: &str) -> String {
    match key {
        ...
        "enter_country" => "国名 (2文字コード):".to_string(),
        ...
        "enter_pfx_password" => "PFXパスワード (空でパスワードなし):".to_string(),
        "files_exist_warning" => "警告: 以下のファイルが既に存在します:".to_string(),
        "overwrite_prompt" => "上書きしますか? (y/N):".to_string(),
        "aborted" => "中断しました。".to_string(),
        _ => key.to_string(),
    }
}
